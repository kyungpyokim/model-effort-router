...
def fast_path_for(task_type: str, level: str, risk_tier: str, facts: dict[str, str], check_available: bool) -> str | None:
    ...
    Precondition: ``"inspect"`` is only correct because ``route()`` has already promoted any risky or
    high-level inspect to ``review``; a caller must apply the same promotion before asking."""
