...
| D9 | Any change to pinned instruction text (review contract) is a schema bump. | `validate_step_instructions` rebuilds and compares instruction text exactly; wording is part of the route contract. |
...
**Reviewer contract (pinned text, schema 8).**
...
`FAILURE` sits immediately above it. Adding a required line changes the pinned review
instructions => `SCHEMA_VERSION` 7 -> 8 and `SUPPORTED_ROUTE_SCHEMA_VERSIONS += (8,)`.
...
**Backward compatibility.** A schema ≤ 7 review that FAILs without a `FAILURE` line is treated as
`implementation` and logged. Schema 8 and later require the line; a missing line on schema 8 is
`EXIT_NO_VERDICT` (11), the same fail-closed treatment a missing `VERDICT` already gets.
