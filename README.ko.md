...
`unknown`과 모호성(ambiguity)은 별개의 계약입니다. **모호성 게이트**는 코드 변경 작업의 요청 텍스트를 읽습니다. 동작(operation)과 구체적 대상(`retry_policy`, `api/models.py`)을 모두 명시하면 `clear`, 둘 중 하나만 명시하면 `partial`이 되어 플래너가 가정(assumption)을 명시해야 하며, 둘 다 없으면 `ambiguous`로 판단해 추측 대신 재작성 요청을 stderr에 쓰고 종료 코드 `3`으로 끝납니다(아무것도 실행되지 않음). 게이트는 레벨·티어·모델·effort·단계 구성을 바꾸지 않으며, 읽기 전용 `design`/`review`/`inspect` 작업은 텍스트 형태로 게이트되지 않습니다. 이미 컨텍스트로 확정된(settled) 라우트는 재작성을 요구받지 않습니다: 사용자가 축을 고정했거나(`--task-type`과 `--level`/`--critical` 동시 지정), 수동 질문에서 축을 선택했거나, 세션이 이 버전이 기록한 모호성 마커를 가진 저장 라우트를 재사용한 경우입니다. `ambiguous` 라우트는 저장되지 않으며, 마커가 없는 이전 버전의 record는 다시 게이트됩니다. 사전 분류 실패 시에는 기존과 같이 안전한 폴백과 자체 종료 코드를 보고합니다.
