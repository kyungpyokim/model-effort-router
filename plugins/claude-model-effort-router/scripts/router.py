...
Authorization or permissions, in the three facts above, is decided by access control boundaries (user authentication, RBAC, ACL, privilege, tenant isolation, credentials, or customer data isolation), not by general workflow control or UX confirmations: model execution approval, CLI confirmation prompts, asking the user before running a tool or command, workflow approval gates, or flags such as --approved are general workflow/runtime control, NOT authorization, permissions, or security changes.
