        ...
        with contextlib.nullcontext(directory):
            if platform == "codex":
                launch = [
                    ...
                    "--sandbox", "read-only", "--cd", directory, "--skip-git-repo-check",
                    "--output-schema", str(schema_path), "--model", model,
                ]
                if effort:
                ...
                launch = [executable, "-p", "--model", model]
                if effort:
                    launch.extend(["--effort", effort])
                if repo_path is not None:
                    launch.extend(["--add-dir", str(repo_path)])
                launch.extend([
                    "--output-format", "json", "--json-schema", json.dumps(CLASSIFIER_SCHEMA),
                    "--safe-mode", "--tools", "Read,Glob,Grep" if repo_path is not None else "",
                    "--permission-mode", "plan", "--no-session-persistence", prompt,
                ])
                unwrap = lambda raw: json.loads(raw)["structured_output"]
            else:
                launch = [executable, "--model", model]
                if effort:
                    launch.extend(["--effort", effort])
                if repo_path is not None:
                    launch.extend(["--add-dir", str(repo_path)])
            ...
            try:
                proc = subprocess.run(launch, text=True, capture_output=True, check=False, timeout=timeout, cwd=directory)
