...
| `changes_trust_boundary` | yes / no / unknown | Designs, changes, or decides where trust is established or delegated between components, services, tenants, or principals (service-to-service auth, token propagation, permission delegation, isolation), including whether to move it. This fact is about the work the task asks you to do, not the artifact under review: reviewing existing boundary code — including a pull request that adds or changes a boundary, isolation, or allowlist rule — moving code within one trust zone, or a rename, extraction, or refactoring whose trust impact is unchanged is no |
...
| `silent_failure_material_harm` | yes / no / unknown | yes only when BOTH are evident: a mistake goes unnoticed (no error, alert, or failing test) AND causes data loss or corruption, wrong money movement, security exposure, or cross-service inconsistency through a described or necessarily implied mechanism (persisted-data write/delete, money movement, auth/access change, unchecked cross-service propagation); mechanical/docs/tests/cosmetic/read-only/single-component visible-failure work is no; hypothetical worst cases are no |
