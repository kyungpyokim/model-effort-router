...
| `crosses_service_boundary` | yes / no / unknown | The task changes behavior, contracts, communication, or data flow across independently deployed services/processes (inter-service API, RPC/message contract, service-to-service dependency, coordination semantics). Changes confined to modules, packages, components, or call sites within one service, mere mention of multiple services, or implementation behind an unchanged cross-service contract is no |
