...
- **Runtime enforcement.** `scripts/pipeline.py` runs the chain: plan -> implement -> deterministic test -> merged review. Launchers always use it; it hands a single interactive stage directly to the terminal, while every two-stage route captures planner stdout and atomically installs `plan.json` before implementation. The route JSON (schema v7) holds only who does each role (`steps`, plus a `pipeline` block: `review`, `replan`, `limits`, `task`); where the run is (phase, fix and review counters) lives in `state.json` in the run's work directory, never in the route.
...
- **Stage permissions.** Claude implement and fix stages use `--permission-mode acceptEdits`; Claude planner, re-plan, and review stages use `dontAsk` with Edit, Write, NotebookEdit, Bash, and MCP denied. Planners return JSON on stdout and cannot write the plan file. Codex uses `--sandbox read-only|workspace-write --ask-for-approval never`; Antigravity uses `--mode plan|accept-edits --sandbox`. These flags apply to interactive commands too. The prompt follows `--` so it is never read as a tool name. `--dangerously-skip-permissions` is never used. Because Bash is not pre-approved for Claude implementers, the launcher's test gate runs shell checks.
- **Route-file argv grammar.** A route file's `command` argv is accepted only in the shapes this router generates, for every schema version, in both `router.py --route-file` and `pipeline.py` (violations exit 2). Schema v7 is current and its commands must retain their generated native permissions, including interactive commands. Schema v6 remains replay-compatible under the pre-v7 native permission grammar; v6 and v7 also rebuild and verify each stage's instructions, including the Codex `developer_instructions` value, the Claude Code/Antigravity prompt instruction block and verification handoff, and the Autobahn scope guard when a security flag is set. Routes older than v6 use their legacy instruction text and are checked against the older grammar only. Model ids are restricted to a strict charset (Antigravity display names may contain spaces and parentheses). The rebuild inputs and each step's model and effort remain self-declared for replay, so a custom-config route can still be replayed; pinning them requires reclassification or a signature. The fix, re-plan and re-implement stages take their scope guard from `risk_flags`, never from the top-level `scope_guard` block. A step whose instructions cannot be rebuilt on the current install is refused as unverifiable. Multi-stage routes may not carry interactive (TUI) steps.
- **Fail-closed checks.** A two-stage artifact must be the same absolute router-owned `codex-route-*/plan.json` in both step output and input. The parent captures planner stdout, validates it, and atomically installs that file; invalid stdout stops the run (exit 13) without replacing an earlier plan. In a git work tree (diffed against the HEAD seen when the run started, so commits count), an implementer that changed nothing fails the review without spending a review call, so it is fixed or re-planned like any other review FAIL.
