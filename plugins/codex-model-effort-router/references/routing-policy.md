...
| `changes_public_api_contract` | yes / no / unknown | The task explicitly changes a stable interface consumed outside the implementation boundary (public HTTP/GraphQL request or response contract, exported SDK/library API, or user-facing CLI command/flag semantics). Adding a new endpoint consumed only by your own frontend, internal function signatures, private module interfaces, implementation behind an unchanged external contract, or mere mention of "API"/"interface"/"endpoint" without an external contract change is no |
