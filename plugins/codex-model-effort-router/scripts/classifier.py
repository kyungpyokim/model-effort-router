...
- changes_trust_boundary: yes when the work designs, changes, or decides where trust is established or delegated between components, services, tenants, or principals (service-to-service authentication, token propagation, permission delegation, isolation boundaries), including deciding whether to move such a boundary. This fact is about the work the task asks you to do, not the artifact under review: reviewing existing boundary code — including a pull request that adds or changes a boundary, isolation, or allowlist rule — is no here (covered by reviews_security_sensitive_code); moving code inside one trust zone is no, and so is a rename, extraction, or refactoring whose trust impact is unchanged.
...
- silent_failure_material_harm: yes only when BOTH are evident from the task text: (a) a mistake could go unnoticed (no error, alert, or failing test), AND (b) it causes material harm such as data loss or corruption, wrong money movement, security exposure, or cross-service inconsistency through a mechanism the task describes or necessarily implies (persisted-data write or delete, money movement, auth or access change, unchecked cross-service propagation). Answer no for mechanical, refactoring, documentation, test, cosmetic, or read-only inspect/design/review work that changes nothing, for single-component fixes with visible failures, and for any merely hypothetical worst case; unknown only when the area is plausibly involved but the text cannot settle it.
