...
"silent_failure_material_harm": {
    ...
    "changes_security_or_payment_logic": {
        "true": "Authentication, authorization, secrets, cryptography, or payment behaviour changes: the work changes who may do what, or how credentials, keys, tokens, money, pricing, or settlement behave, including designing or implementing a signature, custody, settlement, or key-rotation mechanism.",
        "false": "The task only moves, splits, renames, extracts, documents, or reviews such code without changing its behaviour. This fact is about the work the task asks you to do, not about the code or change under review: review or audit work changes no behaviour itself, so reviewing or judging a security pull request — even one that alters authentication, cryptography, or payment behaviour, or that adds, removes, or retunes parameters, filters, or thresholds of such code — is no here, and is covered by reviews_security_sensitive_code and security_domain instead; extracting an auth module into its own service with the same behaviour is no here, since changes_trust_boundary judges that move separately. Purely additive tooling, or a new computational pipeline such as a key-derivation or proof-verification component or an audit log, that leaves existing credentials, keys, and payments behaving as before is no. A task that names a security area without saying what is wrong (for example \"fix the login problem\") is unknown, not yes.",
    },
